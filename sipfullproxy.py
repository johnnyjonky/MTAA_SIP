...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        self.debugRegister()
        self.sendResponse("200 VIBAVENE OK")
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 DOSTE VI (Bad request)")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 NEPISTE MI KET - Temporarily Unavailable")
        else:
            self.sendResponse("500 CERKA PLACE AKO DAST LEBO Server Internal Error")
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 TAKTO TO NEBUDE")
        else:
            self.sendResponse("500 CERKA PLACE AKO DAST LEBO Server Internal Error")
